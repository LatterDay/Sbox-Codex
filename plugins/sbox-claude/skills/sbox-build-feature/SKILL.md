...
| s&box Razor `@if` / root-`class` / `@Prop`-as-text bindings don't re-render reliably | Toggle a panel's visibility imperatively in `OnUpdate`: `Panel.Style.Display = DisplayMode.Flex/None`. For DYNAMIC text use `<label @ref="X">` + `private Label X;` + `X.Text = …` (a `@Prop` text-content binding can stay stale/empty → an empty div renders invisible even when shown). STATIC markup text re-renders fine. |
