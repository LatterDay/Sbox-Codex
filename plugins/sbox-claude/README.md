...
|---|---|
| **MCP server registration** (`sbox` from npm) | 151 tools / 144 editor handlers to drive the s&box editor — GameObjects, scripts, scenes, components, physics, networking, UI, world-gen, lighting & atmosphere, characters, scene layout, navmesh & spatial queries, particles, scatter & object utilities, self-diagnosis, console/C# execution, live docs search, type discovery |
...
Claude should invoke `mcp__sbox__get_bridge_status` and report whether the bridge addon is connected (you'll see `connected: true, handlerCount: 144` if the addon side is also installed and s&box is running).
