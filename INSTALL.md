...
You should get back: `connected: true`, `handlerCount: 144`. (That's the count of C# handlers inside the editor. The MCP server exposes **151** tools total — a handful, like `read_log` / `get_compile_errors` / the docs-search tools, run MCP-server-side and don't need an editor handler.) Then try:
